...
"jdbc:postgresql://cop4710-postgresql.cs.fiu.edu:5432/fall16_adubu002?user=fall16_adubu002&password=2247356";
	...
	public Results(){
		try{
			...
			"SELECT * FROM courses INNER JOIN faculties ON courses.instructor = faculties.faculty_id WHERE description LIKE ?");
...
<table class="centerTable" border="1"> 
	<tbody>
		...
		<tr>		
			<td><% out.print(searchResults.getInt("course_id")); %></td>		
			<td><% out.print(searchResults.getString("description")); %></td>		
			<td><% out.print(searchResults.getString("level")); %></td>		
			<td><% out.print(searchResults.getString("name")); %></td>		
			<td><% out.print(searchResults.getString("semester")); %></td>		
		</tr>
